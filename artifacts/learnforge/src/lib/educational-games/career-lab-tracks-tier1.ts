...
> = {
  ...
  "cloud-computing": [
    {
      id: "cloud-ec2-recovery",
      title: "AWS CLI — EC2 Recovery & Deploy",
      description:
        "On-call alert: production instance stopped. Use the AWS CLI to verify identity, start the instance, confirm health checks, and redeploy static assets to S3.",
      gameType: "terminal-workspace",
      duration: "10–15 min",
      domain: "AWS DevOps",
      ...
      content: {
        terminal: {
          ...
          brief:
            "Monitoring alert: prod-web-01 (i-0abc12345def67890) is in a stopped state and the site is down. Use the AWS CLI to confirm you're on the right account, restart the instance, verify it passes health checks, and redeploy the frontend build to S3.",
          hostname: "devops-jumpbox",
          prompt: "$ ",
          initialOutput: [
            "PagerDuty alert: prod-web-01 DOWN — HTTP health check failed",
            "Instance i-0abc12345def67890 state: stopped",
            "AWS CLI configured. Begin recovery.",
          ].join("\n"),
          steps: [
            {
              instruction: "Verify the active AWS account and IAM identity before making changes.",
              expectedCommand: "aws sts get-caller-identity",
              hint: "aws sts get-caller-identity",
              notes: "Always verify identity before a production change — prevents accidental actions in the wrong account or assumed role.",
            },
            {
              instruction: "List EC2 instances to confirm the target instance and its current state.",
              expectedCommand: "aws ec2 describe-instances",
              hint: "aws ec2 describe-instances",
              notes: "describe-instances returns all instances including stopped ones. Check State.Name — 'stopped' means the OS halted cleanly, 'terminated' is permanent.",
            },
            {
              instruction: "Start the stopped production instance.",
              expectedCommand: "aws ec2 start-instances --instance-ids i-0abc12345def67890",
              hint: "aws ec2 start-instances --instance-ids i-0abc12345def67890",
              notes: "start-instances triggers a boot cycle on the underlying hypervisor. The instance moves from stopped → pending → running within ~30s.",
            },
            {
              instruction: "Check the instance status to confirm both system and instance checks pass.",
              expectedCommand: "aws ec2 describe-instance-status --instance-ids i-0abc12345def67890",
              hint: "aws ec2 describe-instance-status --instance-ids i-0abc12345def67890",
              notes: "AWS runs two health checks: system (hypervisor/host hardware) and instance (OS/reachability). Both must show 'ok' before the instance is production-ready.",
            },
            {
              instruction: "Sync the frontend build artifacts to the production S3 bucket.",
              expectedCommand: "aws s3 sync ./dist s3://prod-static-assets --delete",
              hint: "aws s3 sync ./dist s3://prod-static-assets --delete",
              notes: "s3 sync only uploads changed files (uses ETag/size comparison). --delete removes stale files from the bucket that no longer exist locally.",
            },
          ],
          commandOutputs: {
            "aws sts get-caller-identity": [
              ...
              "    \"Arn\": \"arn:aws:iam::123456789012:user/devops-oncall\"",
              "}",
              "",
              "✓ Account 123456789012 — prod environment confirmed.",
            ].join("\n"),
            "aws ec2 describe-instances": [
              ...
              "      \"State\": { \"Name\": \"stopped\" },",
              "      \"PublicIpAddress\": \"54.210.32.18\",",
              "      \"Tags\": [{ \"Key\": \"Name\", \"Value\": \"prod-web-01\" }]",
              "    }]",
              "  }]",
              "}",
              "",
              "→ prod-web-01 is STOPPED. Proceed to start.",
            ].join("\n"),
            "aws ec2 start-instances --instance-ids i-0abc12345def67890": [
              ...
              "}",
              "",
              "→ Instance transitioning: stopped → pending → running (~30s).",
            ].join("\n"),
            "aws ec2 describe-instance-status --instance-ids i-0abc12345def67890": [
              ...
              "}",
              "",
              "✓ 2/2 checks passed — instance is healthy and running.",
            ].join("\n"),
            "aws s3 sync ./dist s3://prod-static-assets --delete": [
              "upload: dist/index.html to s3://prod-static-assets/index.html",
              "upload: dist/assets/main.abc123.js to s3://prod-static-assets/assets/main.abc123.js",
              "upload: dist/assets/main.abc123.css to s3://prod-static-assets/assets/main.abc123.css",
              "delete: s3://prod-static-assets/assets/main.old456.js",
              "",
              "✓ 3 files uploaded, 1 stale file deleted.",
              "CloudFront invalidation recommended: aws cloudfront create-invalidation --distribution-id EXAMPLEID --paths '/*'",
            ].join("\n"),
          },
        },
      },
